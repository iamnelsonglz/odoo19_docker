...
class CFDIPaid(models.Model):
    _name = 'odoo.cfdi.paid'
    _description = 'CFDI Paid'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _rec_name = 'fiscal_folio'
    _order = 'fiscal_folio asc'

    fiscal_folio = fields.Char(string="Fiscal Folio", required=True)
    ...
    cfdi_issued_id = fields.Many2one('odoo.cfdi.issued', string='CFDI issued related', ondelete='cascade', index=True, required=False, tracking=True)
    ...
    def unlink(self):
        for rec in self:
            ...
            return super(CFDIPaid, self).unlink()
    ...
    def _onchange_xml_file(self):
        ...
        try:
            ...
            related_document = self._get_element_tree(tree, 'DoctoRelacionado')
            ...
            if related_document is not None:
                document_id = related_document.attrib.get('IdDocumento')
                if document_id:
                    issued_record = self.env['odoo.cfdi.issued'].search([('fiscal_folio', '=', document_id),('status', '=', 'emitted')], limit=1)
                    if issued_record:
                        self.cfdi_issued_id = issued_record
